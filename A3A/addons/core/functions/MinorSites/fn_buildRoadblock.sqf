...
private _bunkerType = ["Land_SPE_Sandbag_Nest", "Land_BagBunker_01_Small_green_F"] select _isSPE;
...
{
    _buildings pushBack [typeof _x, getPosWorld _x, vectorUp _x, vectorDir _x];
...
// war tier dependence?
private _troopQuality = [1 + random 0.5, 2] select (_side == Invaders);
...
if (random 15 <= tierWar + 3) then {
    private _type = selectRandom (_faction get "staticsMGs");
