...
while {_totalReinf > 0} do
{
    ...
    if (_rebelsNear or _rebelSpawners inAreaArray [markerPos _source, 1000, 1000] isNotEqualTo [] or ((markerPos _marker distance2d markerPos "Synd_HQ" < (distanceMission * 2.0)) and ((count units Invaders + count units Occupants) < (30 + random 120)))) then {
